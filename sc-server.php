...
if (($fp = fopen("sc-data.dat", "r")) !== false){
    ...
    for($m=1;$m<7;$m++){
        $xoff = 21 + (($m-1) * 266);
        echo '<div class="box4" style="left: '.$xoff.'px; top: 40px; width: 268px;">'.
        ...
        for ($dd = 1; $dd < 32; $dd++) {
            if ($m == intval( date('n', mktime(0,0,0,$m,$dd,$y)))){
                ...
                for($s=0;$s<8;$s++){
                    ...
                    $bor = ($s == 7) ? " border-right: solid black 2px;" : "";
    ...
    for($m=7;$m<13;$m++){
        $xoff = 21 + (($m-7) * 266);
        echo '<div class="box4" style="left: '.$xoff.'px; top: 1068px; width: 268px;">'.
            '<span class="textcentered month">'.date('F', mktime(0,0,0,$m,1,$y)).
            '</span></div>';
        for ($dd = 1; $dd < 32; $dd++) {
            if ($m == intval( date('n', mktime(0,0,0,$m,$dd,$y)))){
